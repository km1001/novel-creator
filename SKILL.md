...
`novel-creator` 是小说生产总控台，不是单纯的“写一段正文”工具。

它的职责只有四件事：
1. 判断当前任务属于新建、续写还是救稿
2. 维护 `plan/` 与 `memory/`，保证长篇不穿帮
3. 在开写前完成**文风预热**，先学题材笔触再落笔
4. 编排调用其他技能，而不是把所有事情都在一个文件里硬做完
...
### 输出前的处理
- 默认做一轮轻量按题材去 AI 润色
- 如果用户要求更自然、更像真人写的、或更像某类小说，优先调用 `novel-humanizer`
...
7. 如有新增，再更新地点、物品、伏笔、错误记录
8. 运行 `scripts/check_chapter_wordcount.py`
9. 当一个 unit 或关键事件完成后，必须做一次“合理性复盘”
